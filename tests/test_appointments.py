...
class AppointmentTestCase(unittest.TestCase):
    ...
    def get_result(self, endpoint):
        res = self.client.get(endpoint, headers={'Authorization': f'Bearer {self.token}'})
    ...
    def get_post_result(self, endpoint):
        res = self.client.post(endpoint, headers={'Authorization': f'Bearer {self.token}'})
        result = json.loads(res.data.decode())
        return res, result

    def test_get_appointments_by_date_range(self):
        """Test that appointments can be filtered by date range."""
        res, result = self.get_result('/get_appointments?start=2022-06-03&end=2022-06-06')
    ...
    def test_get_appointments_by_multiple(self):
        ...
        res, result = self.get_result('/get_appointments?start=2022-06-03&end=2022-06-06&specialisms=Addiction&type=one-off')
        self.assertEqual(result['message'], "Appointments found: 1")
        self.assertEqual(result['appointments'], [{'duration': 60.0,'therapist': 'John Smith','type': 'one-off'}])
        self.assertEqual(res.status_code, 200)
    ...
    def test_add_appointment(self):
        ...
        res, result = self.get_post_result('/add_appointment?start=2022-06-20 12:41&duration=60&type=one-off&therapist_id=1')
    ...
    def test_cant_add_appointment_with_invalid_start(self):
        ...
        res, result = self.get_post_result('/add_appointment?start=2022-05-06&duration=60&type=one-off&therapist_id=1')
        self.assertEqual(result['message'], "Invalid start time or duration.")
        self.assertEqual(res.status_code, 400)
        res, result = self.get_post_result('/add_appointment?start=2022-05-06 12:41&duration=fail&type=one-off&therapist_id=1')
        self.assertEqual(result['message'], "Invalid start time or duration.")
        self.assertEqual(res.status_code, 400)

    def test_cant_add_without_therapist(self):
        """Test that an appointment cannot be added without a valid therapist id."""
        res, result = self.get_post_result('/add_appointment?start=2022-05-06 12:41&duration=60&type=one-off&therapist_id=7')
    ...
    def test_cant_add_appointment_in_past(self):
        ...
        res, result = self.get_post_result('/add_appointment?start=2022-05-06 12:41&duration=60&type=one-off&therapist_id=1')
