...
class AuthTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.bad_email = json.dumps({
            'email': 1234,
            'password': "test_password"
        })
        self.bad_password = json.dumps({
            'email': "test@example.com",
            'password': 1234
        })
    ...
    def test_registration(self):
        ...
        res = self.client.post('/register', json=self.user_data, headers={'Content-Type': 'application/json'})
        result = json.loads(res.data.decode())
        self.assertEqual(result['message'], "Successfully registered new user: test@example.com")
        self.assertEqual(res.status_code, 201)

    def test_registration_bad_json(self):
        """Test user registration fails gracefully when bad json sent."""
        res = self.client.post('/register', json=self.bad_json, headers={'Content-Type': 'application/json'})
        result = json.loads(res.data.decode())
    ...
    def test_already_registered_user(self):
        ...
        res = self.client.post('/register', json=self.user_data, headers={'Content-Type': 'application/json'})
        self.assertEqual(res.status_code, 201)
        second_res = self.client.post('/register', json=self.user_data, headers={'Content-Type': 'application/json'})
    ...
    def test_bad_value_type(self):
        ...
        # Test bad email
        res = self.client.post('/register', json=self.bad_email, headers={'Content-Type': 'application/json'})  
        result = json.loads(res.data.decode())
    ...
    def test_user_login(self):
        ...
        res = self.client.post('/register', json=self.user_data, headers={'Content-Type': 'application/json'})
        self.assertEqual(res.status_code, 201)
        login_res = self.client.post('/login', json=self.user_data, headers={'Content-Type': 'application/json'})
    ...
    def test_login_bad_json(self):
        ...
        res = self.client.post('/login', json=self.bad_json, headers={'Content-Type': 'application/json'})
        result = json.loads(res.data.decode())
        self.assertEqual(result['message'], "Invalid JSON data supplied.")
        self.assertEqual(res.status_code, 400)